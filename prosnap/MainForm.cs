            ...
            if (booladd == false)
            {
                if (e.RowIndex != -1)
                {
                    if (dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString() == "received")
                    {
                        ...
                        String extension = System.IO.Path.GetExtension(filena);
                        String sFileName_save = filena.Substring(0, filena.Length - extension.Length);
                        string fn = filena.Substring(filena.LastIndexOf(('\\')) + 1);
                        string resultpath = ConfigurationSettings.AppSettings["ReceivedFile"] + fn.ToString();
